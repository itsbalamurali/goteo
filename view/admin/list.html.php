        ...
        <?php foreach ($this['data'] as $item) : ?>
            ...
            <?php foreach ($this['columns'] as $key=>$label) : ?>
                <?php if ($key == 'translate') : ?>
                    <td width="5%"><?php if ($translator) : ?><a href="/translate/<?php echo $this['model'].'/edit/'.$item->id; ?>" >[<?php echo Text::_("Translate")?>]</a><?php endif; ?>
                    </td>
                <?php elseif ($key == 'remove') : ?>
                    <td width="5%"><a href="<?php echo $this['url']?>/remove/<?php echo (is_object($item)) ? $item->id : $item['id']; ?>" onclick="return confirm('Are you sure you want to delete this record?');">[<?php echo Text::_("Remove"); ?>]</a></td>
